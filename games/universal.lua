...
local hash = loadstring(downloadFile('catnext/libraries/hash.lua'), 'hash')()
...
entitylib = loadstring(downloadFile('catnext/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {
	alreadychecked = {},
	customtags = {},
	tagcallback = {},
...
run(function()
	...
	function whitelist:announce(text)
		...
		if success then
			return sendToast({
				toastTitle = text,
				iconImage = getcustomasset('catnext/assets/new/vape.png'),
				swipeUpDismiss = true,
		...
		icon.Image = getcustomasset('catnext/assets/new/vape.png')
	...
	function whitelist:update(first)
		...
		if not first or whitelist.textdata ~= whitelist.olddata then
			if not first then
				whitelist.olddata = isfile('catnext/profiles/whitelist.json') and readfile('catnext/profiles/whitelist.json') or nil
			end
			...
			if whitelist.textdata ~= whitelist.olddata then
				...
				pcall(function()
					writefile('catnext/profiles/whitelist.json', whitelist.textdata)
				end)
			end
		end
	end
...
run(function()
    ...
    local function Added(ent)
    	...
    	arrow.BorderSizePixel = 0
    	arrow.Visible = false
    	arrow.Image = getcustomasset('catnext/assets/new/arrowmodule.png')
    	arrow.ImageColor3 = entitylib.getEntityColor(ent) or Color3.fromHSV(Color.Hue, Color.Sat, Color.Value)
    	arrow.Parent = Folder
    	Reference[ent] = arrow
    end
...
run(function()
    ...
    Radar = vape:CreateOverlay({
    	Name = 'Radar',
    	Icon = getcustomasset('catnext/assets/new/radaricon.png'),
    	Size = UDim2.fromOffset(14, 14),
    	Position = UDim2.fromOffset(12, 13),
    	Function = function(callback)
    		if callback then
...
run(function()
    ...
    SessionInfo = vape:CreateOverlay({
    	Name = 'Session Info',
    	Icon = getcustomasset('catnext/assets/new/textguiicon.png'),
    	Size = UDim2.fromOffset(16, 12),
    	Position = UDim2.fromOffset(12, 14),
    	Function = function(callback)
    		if callback then
    ...
    Hide = SessionInfo:CreateTextList({
    	Name = 'Blacklist',
    	Tooltip = 'Name of entry to hide.',
    	Icon = getcustomasset('catnext/assets/new/blockedicon.png'),
    	Tab = getcustomasset('catnext/assets/new/blockedtab.png'),
    	TabSize = UDim2.fromOffset(21, 16),
    	Color = Color3.fromRGB(250, 50, 56)
    })
    SessionInfo:CreateColorSlider({
