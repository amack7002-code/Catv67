...
local drawingactor = loadstring(downloadFile('catnext/libraries/drawing.lua'), 'drawing')(...)
local redline = {Teams = {}}
...
local function warningRoutine(hash)
	local path = 'catnext/profiles/agreementhash.txt'
...
if not select(1, ...) then
	if run_on_actor then
		...
		task.spawn(function()
			...
			local executionString = "loadfile('catnext/main.lua')("..drawingactor..")"
