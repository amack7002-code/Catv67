...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/MaxlaserTech/CatV6/'..readfile('catnextwrite/profiles/commit.txt')..'/'..select(1, path:gsub('catrewrite/', '')), true) end)
...
local sessioninfo = vape.Libraries.sessioninfo
	local vm = loadstring(downloadFile('catnextwrite/libraries/vm.lua'), 'vm')()
